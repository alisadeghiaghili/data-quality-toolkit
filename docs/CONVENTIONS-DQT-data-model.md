...
Configs MUST be validated via typed models (e.g. Pydantic) before pipeline runs.

---

## 3. Storage Schema (Monitoring & UI)

DQT uses a lightweight store (e.g. SQLite) to persist metrics and issues for
monitoring and UI.
...
- `score REAL`
- `value REAL`
- `metadata JSON` (or TEXT with JSON)

Indexes:

- composite index on `(run_id, schema_name, table_name, column_name, dimension)`.
...
- composite index on `(run_id, severity, dimension)`.

These tables back:

- monitoring (trends over time),
- UI dashboards (overview, explorer, issue views),
- report generation.

Implementation can choose SQLite, Postgres, or another DB backend, but the schema
above must stay conceptually stable.

---

## 4. Security Conventions

- Credentials SHOULD come from environment variables or secure config files;
