...
| F3 | **Diagnostics** — all six canonical dimensions with structured issue objects | NOT MET | **PARTIAL** | **Four of six measured**: `completeness`, `uniqueness` (distinct against non-NULL), `consistency` (spellings that collapse under case and whitespace folding) and `referential_integrity` (F2). Each produces a metric whether or not it finds anything, so "measured and fine" is distinguishable from "not measured". `validity` and `timeliness` remain. |
