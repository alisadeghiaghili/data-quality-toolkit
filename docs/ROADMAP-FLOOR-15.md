...
| 3 | **`F14`** `dqt check` and `dqt serve` | M | `serve` is what makes the dashboard startable without Python — and the only way the loopback-bind rule stops being a docstring nobody reads and becomes a refusal. | **Done 2026-09-07.** The refusal is real: a reachable bind address exits 3 before anything binds. `check` computes no statistics, and checking nothing exits 3 rather than 0. |
| 4 | **`DQT-09`** exception hierarchy | S | The only open authoritative task. Not a feature, so it follows the ones that are. | **Done 2026-09-07**, ahead of `F14` because `serve`'s refusal needed a typed error and inventing a one-off to replace later would have been worse. |
...
Wave 1 is complete: **8 of 15**, and the authoritative roadmap's DQT track is
now fully closed.
